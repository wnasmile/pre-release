...
function initGifDrag(gif) {
  ...
  const getIdleSrc = () => {
    const theme = document.documentElement.getAttribute("theme") || DEFAULT_THEME;
    return getThemeGifSrc(theme, "searching");
  };
  ...
  gif.addEventListener("mousedown", (e) => {
    if (dropping) return;
    dragging = true;
    gif.src = "assets/themes/classic/held.gif";
  ...
  document.addEventListener("mouseup", () => {
    ...
    gif.src = "assets/themes/classic/drop.gif";
...
(() => {
  ...
  // FIX: was ROOT_GIFS.loaded (the object), should be the .src string.
  const LOADED_GIF_URL = ROOT_GIFS.loaded.src;
  ...
  function runLoaderSequence() {
    ...
    const loadedSrc = getThemeGifSrc(_getTheme(), "loaded");
  ...
  function runCrashSequence() {
    ...
    const crashSrc = getThemeGifSrc(theme, "crash");
    const dedSrc   = getThemeGifSrc(theme, "ded");
...
<body>
  ...
  function createAssetCards(data) {
    ...
    for (const asset of domOrdered) {
      ...
      if (status === "cooked") card.classList.add("cooked");
    ...
    for (const [pageNum, { promises, cards }] of pagePromiseMap) {
      ...
      Promise.all(promises).then(() => {
        ...
        if (isActive && !activePageHandled) { activePageHandled = true; runLoaderSequence(); }
      });
    }

    if (!pagePromiseMap.has(activePage)) runLoaderSequence();
  ...
  window.reloadAssets = async function () {
    ...
    loader.innerHTML = `<img src="${getThemeGifSrc(_getTheme(), "loading")}" alt="" />`;
